...
email_prog = re.compile(r'^[A-Za-z0-9]+@[A-Za-z0-9]+\.[A-Za-z0-9]+(\.[A-Za-z0-9]+)*$')

def re_email(mail_addr:str):
	return True if email_prog.match(mail_addr) else False
...
def send_simple_message(mailfrom, mailpw, mailto, mailsubject, mailcontent): #寄信
	global message
	str_smtp = "smtp.gmail.com:587"  #主機
	str_account = mailfrom  #帳號
	str_password = mailpw  #密碼
	msg = MIMEText(mailcontent)
	msg["Subject"] = mailsubject  #郵件標題
	mailto1 = mailto  #收件者
	server = SMTP(str_smtp)  #建立SMTP連線
	server.ehlo()  #跟主機溝通
	server.starttls()  #TTLS安全認證
	try:
		server.login(str_account, str_password)  #登入
		server.sendmail(str_account, mailto1, msg.as_string())  #寄信
